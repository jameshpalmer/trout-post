...
<Accordion type="single" collapsible>
  <AccordionItem value="item-1" className="border-none">
    <AccordionTrigger className="gap-2 py-0 pr-2 text-left font-normal text-[#374151] hover:no-underline">
      It's not necessary to understand what a linear solver to understand
      stochastic programming, but if you are genuinely curious, you can click
      here to find out more.
    </AccordionTrigger>
    <AccordionContent>
      <br />
      Think of your FPL decisions as variables: owning a player can be a variable
      with value 1, for example, and not owning them might have value 0. The optimal
      value of a decision variable is not known before the problem is solved. To
      satisfy a linear model, your constraints and objective can contain decision
      variables multiplied by numbers, added to other decision variables multiplied
      by numbers, but nothing more. You cannot, for example, multiply a decision
      variable by a decision variable. This may seem very constraining; how on earth
      are you supposed to adequately describe a problem as complicated as FPL just
      by adding decisions?!. And it is! However, by keeping our models linear, we
      are able to solve them using some incredibly powerful machinery.[^2] Although
      our language for describing FPL is constrained, we can find optimal solutions
      for linear problems - and prove that they truly are optimal - with relative
      ease. Anyway, FPL is, for the most part, a linear game: your decisions (the
      players we own) are multiplied by numbers (how many points they score) and
      added together - although representing things linearly becomes a challenge
...
Ever since I started using a solver for FPL, almost every alteration I've made
to my model has been motivated by my desire to balance grabbing EV with
maintaining the ability to react to change.
